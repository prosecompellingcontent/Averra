...
export function CartPreviewPopup({ items, isVisible, onClose, onNavigateToCart }: CartPreviewPopupProps) {
  ...
  useEffect(() => {
    if (isVisible) {
      console.log('CartPreviewPopup - isVisible:', isVisible);
      console.log('CartPreviewPopup - isMobile:', isMobile);
      console.log('CartPreviewPopup - items length:', items.length);
    }
  }, [isVisible, isMobile, items.length]);
  ...
  // Mobile version - centered in viewport
  if (isMobile) {
    return (
      <div
        className="fixed left-4 right-4 z-[9999] border-2 border-[#301710] shadow-2xl p-4 bg-white transition-all duration-200"
        style={{
          top: '50%',
  ...
  return (
    <div
      className="fixed right-8 z-[9999] w-96 bg-white border-2 border-[#301710] shadow-2xl transition-all duration-200"
      style={{
        top: '100px',
