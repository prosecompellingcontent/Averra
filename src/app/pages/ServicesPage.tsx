...
const digitalProducts = [
  ...
  },
  {
    ...
    scenes: [
      { title: "Scene 1 — The Treatment Moment", detail: "A calm facial appointment scene that captures the treatment table setup, clean linens, and hands-on care clients expect. Soft lighting and a relaxed client position create the kind of environment that feels professional, safe," },
...
export function ServicesPage() {
  ...
  const handleAddToCart = (item: { id: string; name: string; priceNum: number; originalPriceNum?: number; subtitle?: string; type: 'service' | 'digital' | 'membership'; description?: string }) => {
    addItem({
      id: item.id,
      name: item.name,
      price: item.priceNum,
            ...
            >
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
                <div className="lg:col-span-1 space-y-8">
                  ...
                  <button
                    onClick={() => {
                      // Store selected tier in sessionStorage for checkout
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {digitalProducts.map((product) => (
              <div key={product.name} className="bg-white/60 p-8 hover:bg-white transition-all">
                ...
                {/* Service-specific packs with scenes */}
                {'scenes' in product && (
                  <>
                    {/* Tagline - only for products that have it */}
                    {product.description && (
                ...
                {/* Legacy format for older products (Mini AI Stock Pack, Seasonal Visual Drop) */}
                {'bestFor' in product && (
                  <>
                    <p className="text-sm text-[#654331] font-light mb-4 leading-relaxed">
                      {product.includes}
                    </p>
                    <p className="text-xs text-[#654331] italic mb-4" style={{ fontFamily: 'Cormorant, serif' }}>
                      Best for: {product.bestFor}
                    </p>
                    {/* Positioning statement for legacy products */}
                    {product.positioning && (
                      <p className="text-xs text-[#654331] italic mb-6 leading-relaxed" style={{ fontFamily: 'Cormorant, serif' }}>
                        {product.positioning}
                      </p>
